...
test('landing page renders, scrolls and routes into the workspace',async({page},testInfo)=>{
  ...
  await expect(page.getByRole('button',{name:'Open app',exact:true})).toHaveCSS('color','rgb(255, 255, 255)');
  expect(await page.evaluate(()=>document.documentElement.scrollWidth<=window.innerWidth)).toBeTruthy();
  // The hero must not be pinned to one viewport on desktop, and no CTA may be inert.
  if(testInfo.project.name==='desktop'){
    expect(await page.evaluate(()=>document.documentElement.scrollHeight>window.innerHeight+4)).toBeTruthy();
    await page.evaluate(()=>window.scrollTo(0,document.body.scrollHeight));
    expect(await page.evaluate(()=>window.scrollY)).toBeGreaterThan(0);
  }
  expect(await page.evaluate(()=>[...document.querySelectorAll('a')].filter(a=>!a.getAttribute('href')||a.getAttribute('href')==='#').length)).toBe(0);
  await page.screenshot({path:`test-results/${testInfo.project.name}-landing.png`,fullPage:true});
