...
export function parseManifest(filename: string, content: string, source?: Provenance): Graph {
  ...
  function add(
    ...
    if (!exact(version))
      throw new Error(
        `Unresolved version for ${name}. Supply an exact lockfile or resolved dependency tree.`,
      );
    const n = node(eco, name, version, kind);
    n.provenance = [origin];
    graph.nodes.push(n);
    return n;
  }
  if (file === "package-lock.json" || file === "npm-shrinkwrap.json") {
    ...
    if (![2, 3].includes(data.lockfileVersion) || !data.packages?.[""])
      throw new Error("Use npm package-lock v2 or v3 with a packages map.");
    const entries = data.packages as Record<
      string,
      {
        name?: string;
        version?: string;
    ...
    for (const [path, info] of Object.entries(entries)) {
      if (info.link) {
        graph.warnings.push(`Workspace link ${path} was not resolved; graph coverage is partial.`);
        continue;
      }
      const name =
        info.name || (path ? path.split("node_modules/").pop()! : data.name || "application");
      if (!info.version && path) {
        graph.warnings.push(`Unresolved dependency ${name} omitted.`);
        continue;
      }
      const n = add(
        "npm",
        name,
        info.version || data.version || "0.0.0",
        path ? "package" : "service",
      );
      n.scope = info.dev ? "development" : "runtime";
    ...
    for (const line of content.split("\n")) {
      ...
      if (!value || value.startsWith("--hash")) continue;
      const match = value.match(
        /^([A-Za-z0-9_.-]+)(?:\[[A-Za-z0-9_,.-]+\])?==([0-9][A-Za-z0-9.!+_-]*)(?:\s*(?:;.*|--hash=.*|\\))?$/,
      );
      if (!match) {
        graph.warnings.push(
      ...
      const child = add("pypi", match[1], match[2]);
      graph.edges.push({ from: root.id, to: child.id });
    }
    graph.warnings.push(
      "Flat requirements snapshot: all pins are attached to the application. Transitive ancestry and environment markers are not resolved.",
    );
    ...
    for (const group of ["default", "develop"])
      for (const [name, raw] of Object.entries(data[group] || {})) {
        ...
        child.scope = group === "develop" ? "development" : "runtime";
        graph.edges.push({ from: root.id, to: child.id });
      }
    graph.warnings.push(
      "Pipfile.lock is a flat snapshot; direct/transitive ancestry and environment markers are not resolved.",
    );
  } else if (file === "dependency-tree.json") {
    const tree = JSON.parse(content);
    function walk(raw: any, parent?: PackageNode, depth = 0) {
      if (depth > 100 || graph.nodes.length > 10000) {
        graph.warnings.push("Large Maven tree: remaining branches were not expanded.");
        return;
      }
      if (!raw.groupId || !raw.artifactId)
        throw new Error("Expected Maven dependency:tree JSON coordinates.");
      const n = add(
        "maven",
        raw.groupId + ":" + raw.artifactId,
        raw.version,
        parent ? "package" : "service",
      );
      n.scope = raw.scope === "test" ? "development" : "runtime";
    ...
    if (/<!DOCTYPE|<!ENTITY/i.test(content))
      throw new Error("XML declarations with entities or DTDs are not accepted.");
    ...
    const root = add(
      "maven",
      (doc.groupId || doc.parent?.groupId || "local") + ":" + (doc.artifactId || "application"),
      doc.version || doc.parent?.version || "0.0.0",
      "service",
    );
    const deps = doc.dependencies?.dependency;
    ...
    for (const dep of list) {
      ...
      if (typeof version === "string")
        version = version.replace(
          /\$\{([^}]+)\}/g,
          (_: string, key: string) => doc.properties?.[key] || "",
        );
      if (!exact(version)) {
        graph.warnings.push(
          `Unresolved managed version: ${dep.groupId}:${dep.artifactId}. Export dependency-tree.json for full resolution.`,
        );
        continue;
      }
      const child = add("maven", dep.groupId + ":" + dep.artifactId, version);
      child.scope = dep.scope === "test" ? "development" : "runtime";
      graph.edges.push({ from: root.id, to: child.id });
    }
    graph.warnings.push(
      "POM import includes exact direct dependencies only. Parent/BOM mediation, profiles and transitives require dependency-tree.json.",
    );
  } else throw new Error(`Unsupported file. Supported: ${supportedFiles.join(", ")}.`);
  if (!graph.nodes.some((n) => n.kind === "package"))
    graph.warnings.push(
      "No exact package versions could be extracted. This is an incomplete dependency snapshot.",
    );
