...
export async function createApp(
  ...
  app.addHook("onRequest", async (req, reply) => {
    if (
      ...
      req.url === "/api/v1/webhooks/github"
    )
      return;
  ...
  app.post<{ Params: { scanId: string } }>("/api/v1/scans/:scanId/analyze", async (req, reply) => {
    const selection = z
      .object({
        projects: z
          .array(
            z.object({ id: z.string().max(100), name: z.string().trim().min(1).max(100) }).strict(),
          )
          .min(1)
          .max(5000),
      })
      ...
      .parse(req.body);
    try {
      return reply.code(202).send(jobs.analyze(req.params.scanId, selection.projects));
    } catch (error) {
      return reply.code(400).send({ error: (error as Error).message });
    }
  });
  ...
  app.get<{ Params: { scanId: string } }>(
    ...
    async (req) => getScan(req.params.scanId, true).graph,
  );
  app.get<{ Params: { scanId: string } }>(
  ...
  await app.register(async (webhook) => {
    ...
    webhook.post("/api/v1/webhooks/github", async (req, reply) => {
      ...
      if (event !== "push") return reply.code(202).send({ ignored: true });
      let body: any;
      try {
      ...
      if (
        ...
        return reply.code(403).send({ error: "Repository is not enabled for webhook scans." });
      ...
      if (
        !(process.env.GITHUB_INSTALLATION_IDS || "").split(",").map(Number).includes(installationId)
      )
        return reply.code(403).send({ error: "Installation is not allowed." });
      ...
      try {
        ...
        const scan = jobs.create({ mode: "github", repository: repo, ref: sha, installationId });
        store.db.exec("COMMIT");
        return reply.code(202).send({ scanId: scan.id });
      } catch (error) {
  ...
  if (existsSync(resolve("dist/web/index.html"))) {
