...
function ModelIcon({ model, className }: { model: string; className?: string }) {
  return <svg className={className} viewBox="0 0 20 20" fill="none" aria-hidden="true"><path d="M2 7c3-4 5 4 8 0s5 4 8 0M2 13c3-4 5 4 8 0s5 4 8 0" stroke="currentColor" strokeWidth="1.6"/></svg>;
}
...
export const PromptInput = React.forwardRef<HTMLDivElement, PromptInputProps>(
  (
    ...
    const startRecording = useCallback(async () => {
      ...
      if (stream) {
        ...
        if (SpeechRecognition) {
          ...
          console.warn("Speech recognition is unavailable in this browser.");
          stopRecording();
        }
      } else {
        stopRecording();
    ...
    const showArrow = (hasValue || !voiceEnabled) && !isRecording;
    const showStop = isRecording;
    const showMic = voiceEnabled && !hasValue && !isRecording;
        ...
        >
          ...
          >
            ...
            >
              ...
              <button
                type="button" aria-label="Response detail" onMouseDown={(e) => e.preventDefault()} onClick={cycleEffort}
              ...
              <button
                type="button" onMouseDown={(e) => e.preventDefault()} onClick={openFileChooser} hidden={maxAttachments === 0} aria-label="Attach image" disabled={attachments.length >= maxAttachments}
            ...
            <button
              ...
              disabled={disabled || (!hasValue && !voiceEnabled)}
